classdef ELITEOption < handle
    %{
        CLASS DESCRIPTION

        Option of ELITE model
    ...
    methods(Static) 
        function checkInputForTrain(obj, input_)
            ...
            switch numInput
                case 1
                    obj.datawnan = input_{1};
                    input_{1}(isnan(input_{1}))=0;
                    obj.data = input_{1};
            ...
            switch obj.dataType
                ...
                case 'others'
                    error('ELITE is only supported for one-class or binary classification.')
            end
        ...
        function results = checkInputForTest(input_)
            ...
            switch length(tmp_)
                ...
                otherwise
                    error('ELITE is only supported for one-class and binary classification.')
            end
        end
        ...
        function setParameter(obj, parameter)
            ...
            if year_ < 2016 || (year_ == 2016 && version_(5) == 'a')
                error('ELITE V2.1 is not compatible with the versions lower than R2016b.')
            end
            %
        ...
        function displayTrain(obj)
            fprintf('\n')
            fprintf('*** ELITE model training finished ***\n')
            ...
            if obj.numSupportVectors/obj.numSamples > 0.5
                warning('The trained ELITE model may be overfitting.')
            end
            fprintf('\n')
        end
        ...
        function displayTest(results)
            fprintf('\n')
            fprintf('*** ELITE model test finished ***\n')
            fprintf('running time            = %.4f seconds\n', results.runningTime)
            fprintf('number of samples       = %d \n', results.numSamples)
